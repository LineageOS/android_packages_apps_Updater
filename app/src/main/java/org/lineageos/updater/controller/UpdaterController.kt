...
class UpdaterController private constructor(context: Context) {
    ...
    fun notifyInstallProgress(downloadId: String?) {
        val intent = Intent().apply {
            action = ACTION_INSTALL_PROGRESS
            putExtra(EXTRA_DOWNLOAD_ID, downloadId)
        }
    ...
    private fun getDownloadCallback(downloadId: String) = object : DownloadCallback {
        override fun onResponse(headers: DownloadClient.Headers?) {
            ...
            headers?.get("Content-Length")?.also {
                try {
                    val size = it.toLong()
    ...
    fun setUpdatesAvailableOnline(downloadIds: List<String?>, purgeList: Boolean) {
        ...
        for (entry in downloads.values) {
            ...
            if (!online &&
                purgeList && entry.update.persistentStatus == UpdateStatus.Persistent.UNKNOWN
            ) {
    ...
    fun resumeDownload(downloadId: String) {
        ...
        if (update.fileSize > 0 && file.length() >= update.fileSize) {
    ...
    fun pauseDownload(downloadId: String) {
        ...
        downloads[downloadId]?.let {
            it.downloadClient!!.cancel()
            removeDownloadClient(it)
            it.update.status = UpdateStatus.PAUSED
            it.update.eta = 0
            it.update.speed = 0
            notifyUpdateChange(downloadId)
        }
    ...
    private fun deleteUpdateAsync(update: Update) {
        Thread {
            update.file?.let {
                if (!it.delete()) {
                    Log.e(TAG, "Could not delete " + it.absolutePath)
                }
            }
    ...
    val updates
        get() = mutableListOf<UpdateInfo>().apply {
            for (entry in downloads.values) {
                add(entry.update)
            }
        }.toList()

    fun getUpdate(downloadId: String) = downloads[downloadId]?.update

    fun getActualUpdate(downloadId: String?) = downloads[downloadId]?.update

    fun isDownloading(downloadId: String) = downloads[downloadId]?.downloadClient != null

    fun hasActiveDownloads() = activeDownloads > 0

    val isVerifyingUpdate: Boolean
        get() = verifyingUpdates.size > 0

    fun isVerifyingUpdate(downloadId: String) = verifyingUpdates.contains(downloadId)

    val isInstallingUpdate: Boolean
        get() = isInstalling ||
                isInstallingUpdate(applicationContext)

    fun isInstallingUpdate(downloadId: String) = isInstalling(downloadId) ||
    ...
    fun isWaitingForReboot(downloadId: String?) = isWaitingForReboot(applicationContext, downloadId)
