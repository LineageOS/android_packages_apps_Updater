...
class ExportUpdateService : Service() {
    ...
    private inner class ExportRunnable(
        ...
        private val runnableComplete: Runnable,
        private val runnableFailed: Runnable
    ) : Runnable {
        override fun run() {
            try {
                copyFile(contentResolver, source, destination, progressCallBack)
                isExporting = false
                if (!exportThread!!.isInterrupted) {
                    Log.d(TAG, "Completed")
    ...
    private fun startExporting(source: File, destination: Uri) {
        ...
        exportThread = Thread(exportRunnable).apply {
            start()
        }
    }
