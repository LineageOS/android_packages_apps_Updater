...
public class Changelog {
    ...
    private List<ChangelogEntry> mUpdatedChanges = new ArrayList<>();
    private List<ChangelogEntry> mChanges = new ArrayList<>();
    private List<ChangelogEntry> mLabels = new ArrayList<>();

    public Changelog(Context context) {
        mBuildTimestamp = SystemProperties.getLong(Constants.PROP_BUILD_DATE, 0);
    ...
    public void setUpdates(List<UpdateInfo> updates) {
        ...
        addToSortedChangeList(mLabels, new BuildLabel(buildTimestamp,
                mContext.getString(R.string.changelog_current_build_label)));
    }
    ...
    public synchronized void startFetching(ChangelogCallback callback) {
        ...
        mGetThread = new Thread(() -> {
            for (; mRequests < MAX_REQUESTS &&
                    (mLastTimestamp < 0 || mLastTimestamp >= mBuildTimestamp); mRequests++) {
    ...
    private long updateChangelog(String changelogJson) {
        try {
             ...
             * For this reason, updated changes may be positioned incorrectly.
             * Add to the mChanges only those with a submit date newer than
             * "last" and keep the others in mUpdatedChanges.
             */

            while (mUpdatedChanges.size() > 0 && mUpdatedChanges.get(0).getTimestamp() >= last) {
                addToSortedChangeList(mChanges, mUpdatedChanges.remove(0));
            }
            ...
            for (int i = 0; i < res.length(); i++) {
                try {
                    ...
                    if (subject.equals("Automatic translation import")) {
                        continue;
                    }
                    ...
                    if (change.getTimestamp() < last) {
                        addToSortedChangeList(mUpdatedChanges, change);
                    } else {
