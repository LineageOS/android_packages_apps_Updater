...
public class HttpURLConnectionClient implements DownloadClient {
    ...
    private void downloadFileInternalCommon(boolean resume) {
        ...
        mDownloadFileAsyncTask.execute(resume);
    }
    ...
    private class DownloadFileAsyncTask extends AsyncTask<Boolean, Void, Void> {
        ...
        @Override
        protected Void doInBackground(Boolean... resumes) {
            boolean resume = resumes[0];
            try {
                mClient.connect();
                ...
                try (
                        ...
                        OutputStream outputStream = new FileOutputStream(mDestination, resume)
                ) {
                    mTotalBytes = mClient.getContentLength();
                    byte[] b = new byte[8192];
                    int count;
                    while (!mCancelled && (count = inputStream.read(b)) > 0) {
